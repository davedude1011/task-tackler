...
function GetBookworkCode() {
    ...
    if (Code && Code.textContent && Code.textContent.includes("Bookwork code: ")) { return Code.textContent.split("Bookwork code: ")[1] }
    else { return undefined }
}
function SubmitButtonSetup() {
    ...
    for (let i = 0; i < AllButtonBases.length; i++) {
        if (AllButtonBases[i].textContent == "Submit answer" && !AllButtonBases[i].classList.contains("event-listener-added")) {
            ...
            AllButtonBases[i].addEventListener("click", function() { // Submit answer Code:
                let AnswerHTML = document.querySelector("[class*='_QuestionWrapper_']")?.innerHTML
                if (AnswerHTML) {
                    let CheatSheet: Array<any> = JSON.parse(localStorage.getItem("CheatSheet") || '[]');
...
window.addEventListener("click", function() { // runs every time the user clicks anywhere
    setTimeout(function() {
        //if (GetButtonBaseTextContent() == "Watch video") { // user is on the question page
        //    let BookworkCode = GetBookworkCode()
        //    if (BookworkCode) {
        //        localStorage.setItem("MostRecentCode", BookworkCode)
        //    }
        //}
        if (GetButtonBaseTextContent() == "Back") { // user is on the answer page
            SubmitButtonSetup()
        }
    }, 500)
})
    ...
    try {
        if (document.querySelector("[class*='_DialogTitle_']")?.textContent == "Bookwork check") {
            ...
            if (BookworkCode && BookworkCode.length < 20) {
                ...
                //document.querySelector("[class*='_Chip_']")?.remove()
                let EditElement: HTMLElement | null = document.querySelector("[class*='_Subtitle_']")
                if (EditElement) {
...
function AddCalculator() {
    ...
    if (Calculators.length == 0 && QuestionContainerClass != null) {
        ...
        let Container: HTMLElement | null = document.querySelector("[class*='_Activity_']")
    ...
    function ToggleCalculator() {
        let MainCalculator: HTMLElement | null = document.querySelector(".main-calculator")
        let QuestionElement: HTMLElement | null = document.querySelector("[class*='_QuestionContainer_']:not(.main-calculator)")
        if (MainCalculator && QuestionElement) {
            if (MainCalculator.style.display == "none") { // display calculator
                MainCalculator.style.display = "block"
                QuestionElement.style.margin = "auto auto 1rem 1vw"
            }
            else { // hide calculator
    ...
    let CalculatorText: HTMLElement | null = document.querySelector("[class*='_CalculatorInfoContainer_'] [class*='_HiddenAt_']")
    if (CalculatorText) {
        if (CalculatorText.innerHTML != "Toggle<br>Calculator") {
            ...
            let CalculatorIcon: HTMLImageElement | null = document.querySelector("[class*='_CalculatorIcon_']")
            if (CalculatorIcon) {
            ...
            let CalculatorIconOuter: HTMLImageElement | null = document.querySelector("[class*='_CalculatorInfoContainer_']")
...
function SnipeBookworkCheck() {
    // Basically checks if the bookwork check is up, if it is it deletes the code and randomly selects
    // an option, then presses the refresh button, then loops back over if the cycle isnt over...

    let BookworkCode = document.querySelector("[class*='_Chip_']")
    if (BookworkCode && BookworkCode.textContent?.split(" ")[0] == "Bookwork") {
        BookworkCode.remove()

        let FirstOption: HTMLElement | null = document.querySelector("[class*='_WACOption_']")
        if (FirstOption) {
            FirstOption.click()
            setTimeout(function() {
                let AllSubmitButtons = document.querySelectorAll("[class*='_ButtonBase_']")
                for (let i = 0; i < AllSubmitButtons.length; i++) {
                    let SubmitButton = AllSubmitButtons[i] as HTMLElement
        ...
        for (let i = 0; i < AllButtons.length; i++) {
            let RefreshButton = AllButtons[i] as HTMLElement
...
function AddGauthMathLink() {
    ...
    if (!ExistingGauthButton && ButtonClass != null) {
        ...
        NewGauthButton.classList.add(ButtonClass) // at some point make this auto copy other calculator menu button class
...
function CleanMenu() {
    if (window.location.pathname == "/student/homework/" || window.location.pathname == "/student/homework") {
        ...
        for (let i = 0; i < HomeworksContainer.length; i++) {
            let HomeworkMenu: any = HomeworksContainer[i]
...
function Loop() {
    (async function() {
        let PopupData: any = await isPopupDataValueActive("Global");
        if (PopupData["SaveBookworkChecks"]) { BookworkCheckLoop() }
        if (PopupData["AddCalculator"]) { AddCalculator() }
        if (PopupData["GauthMathButton"]) { AddGauthMathLink() }
        if (PopupData["sparx-CleanMenu"]) { CleanMenu() }
        if (PopupData["sparx-FakeCrash"]) { createCrashButton() }
