...
function improveQuestionUi() {
    ...
    for (let i = 0; i < allQuestionButtons.length; i++) {
        let element = allQuestionButtons[i];
    ...
    if (!knowitallninjaData) {
        ...
        if (bodyClass) {
            ...
            for (let i = 0; i < bodyClass.length; i++) {
                ...
                if (classItem.includes("postid-")) {
                    postId = classItem.split("postid-")[1];
                }
                else if (classItem.includes("learndash-cpt-sfwd-quiz-")) {
                    quiz = classItem.split("learndash-cpt-sfwd-quiz-")[1];
                }
                else if (classItem.includes("learndash-cpt-sfwd-courses-")) {
                    course_id = classItem.split("learndash-cpt-sfwd-courses-")[1];
                }
                else if (classItem.includes("learndash-cpt-sfwd-lessons-")) {
                    lesson_id = classItem.split("learndash-cpt-sfwd-lessons-")[1];
                }
                else if (classItem.includes("learndash-cpt-sfwd-topic-")) {
                    topic_id = classItem.split("learndash-cpt-sfwd-topic-")[1];
                }
            }
...
function mainLoop() {
    (async function () {
        ...
        if (PopupData["AddQuestionChecking"]) {
            improveQuestionUi();
        }
        if (PopupData["DevData"]) {
            revealCourseData();
        }
    })();
}
setInterval(mainLoop, 100);
//# sourceMappingURL=knowitallninja.js.map